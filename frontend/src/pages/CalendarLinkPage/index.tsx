...
import { Calendar, Plus, Copy, Check, X, Download } from "lucide-react";
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Distribute dynamic event subscription redirects for various calendar services.
          </p>
        </div>
        <Button onClick={() => setOpen(true)} className="flex items-center gap-1.5">
          <Plus className="h-4 w-4" />
          <span>Create Calendar Link</span>
        </Button>
      </div>
      ...
      {code && (
        <div className="p-4 rounded-2xl border border-lime-brand/25 bg-lime-brand/5 backdrop-blur-md flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground font-semibold">Calendar Link Created</p>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card/65 backdrop-blur-md shadow-sm">
        {loading ? (
          <div className="p-12 text-center text-sm text-muted-foreground">
            ...
            <p className="mt-2 font-medium">Fetching calendar routes...</p>
          </div>
        ) : links.length === 0 ? (
          <div className="p-12 text-center text-sm text-muted-foreground font-medium">
            No calendar links created yet. Click the button above to set your first event link.
          </div>
        ) : (
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-muted/40 border-b border-border">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">Calendar Providers</th>
                    ...
                    <td className="px-4 py-3.5 font-medium">
                      <div className="flex flex-wrap gap-1.5">
                        <Link
                          to={`/c/g/${l.slug}`}
                          target="_blank"
                          className="inline-flex items-center rounded-lg px-2 py-0.5 text-xs font-bold uppercase tracking-wide bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 border border-blue-500/20 transition-colors"
                        >
                          Google
                        </Link>
                        <Link
                          to={`/c/o/${l.slug}`}
                          target="_blank"
                          className="inline-flex items-center rounded-lg px-2 py-0.5 text-xs font-bold uppercase tracking-wide bg-orange-500/10 text-orange-500 hover:bg-orange-500/20 border border-orange-500/20 transition-colors"
                        >
                          Outlook
                        </Link>
                        <Link
                          to={`/c/y/${l.slug}`}
                          target="_blank"
                          className="inline-flex items-center rounded-lg px-2 py-0.5 text-xs font-bold uppercase tracking-wide bg-purple-500/10 text-purple-500 hover:bg-purple-500/20 border border-purple-500/20 transition-colors"
                        >
                          Yahoo
                        </Link>
                        <Link
                          to={`/c/d/${l.slug}`}
                          target="_blank"
                          className="inline-flex items-center gap-1 rounded-lg px-2 py-0.5 text-xs font-bold uppercase tracking-wide bg-lime-brand/10 text-lime-brand hover:bg-lime-brand/20 border border-lime-brand/20 transition-colors"
                        >
